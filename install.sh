...
if [[ -x /usr/local/bin/glances || -x /usr/bin/glances ]]; then
    ...
    # Install libs
    do_with_root pip install --upgrade psutil bottle batinfo https://bitbucket.org/gleb_zhulik/py3sensors/get/tip.tar.gz zeroconf netifaces pymdstat influxdb statsd pystache docker-py pysnmp pika
